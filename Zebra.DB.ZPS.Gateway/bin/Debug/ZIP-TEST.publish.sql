...
GO
PRINT N'Dropping [svc].[Parcel$VerifyForImportUSD]...';


GO
DROP FUNCTION [svc].[Parcel$VerifyForImportUSD];
...
GO
PRINT N'Altering [lc].[Parcel$ShippingPlanVia]...';


GO
-- AaronLiu
ALTER FUNCTION [lc].[Parcel$ShippingPlanVia](@number varchar(40))
RETURNS TABLE
--WITH ENCRYPTION
AS RETURN 
(
	with cteParcel as
	(
		select	ID=x.MatterID, p.Type, ShippingPlan=i.Info
	...
	select	ID, Type, ShippingPlan, Complied=s.Tvp
...
GO
PRINT N'Altering [hub].[Parcel$SummaryForUSD]...';


GO
-- Aimee
ALTER	FUNCTION [hub].[Parcel$SummaryForUSD]()
RETURNS	TABLE
WITH ENCRYPTION
AS RETURN
(
	select	x.Source, x.StateID, x.RcvHubID, RcvHubAlias=t.Alias
	,		TotalParcelCnt  =isnull(count(*), 0)
	,		TotalAddOnSvcCnt=isnull(count(a.OperatorID), 0)
	,		OperationDate   =convert(date, dateadd(hour, t.UtcOffset, x.StatedOn),120)
	from	shpt.Parcel#Base()         x
	join	core.Tenant#Raw()          t on t.ID=x.RcvHubID
	cross	apply core.AddOnSvc#Type() k
	left	join  core.AddOnSvc#Raw()  a on (a.MatterID=x.ID and a.Type=106/*k.Overlabel*/)
	cross	apply core.Source#ID()     s
	cross	apply core.State#ID()      e
	where	x.Source=s.USD and x.StateID=e.CfmGated
	group	by x.Source, x.StateID, x.RcvHubID, t.Alias
	,		convert(date, dateadd(hour, t.UtcOffset, x.StatedOn),120)
)
GO
...
END
GO
PRINT N'Altering [hub].[Parcel$UpdAddOnSvc]...';
...
*/
-- Aimee
ALTER PROCEDURE [hub].[Parcel$UpdAddOnSvc](@slip tvp, @tenancy tvp)
WITH ENCRYPTION
AS
BEGIN
	SET NOCOUNT    ON;
...
GO
/*
	@slip	=Triad<RcvHubID, HasAddOnSvc, OperationDate>
*/
-- Daxia
...
BEGIN
	...
	BEGIN TRY
		...
		declare	@hubID I32,               @hasAddOnSvc bit,               @opDate DT;
		select	@hubID=cast(x.v1 as int), @hasAddOnSvc=cast(x.v2 as bit), @opDate=x.v3
		...
		where	x.StateID in (s.CfmGated) and x.Source=@source
	...
	BEGIN TRY
		...
		begin
			...
			begin
				...
				insert	@postCourierSpec
				(		 MatterID,     SvcClass, PostCourier,   Type)
				select	@matterID, @newSvcClass, t.TrackingNbr, iif(@newSvcClass=@svcClassLW, r.PostCourierPrevious, r.PostCourierPrevious)
