...
GO
PRINT N'Altering [tms].[_SvcType]...';


GO
ALTER TABLE [tms].[_SvcType]
    ADD [Code]         VARCHAR (30) CONSTRAINT [DF_SvcType_Code] DEFAULT ('') NOT NULL,
        [WeightMethod] [dbo].[E8]   CONSTRAINT [DF_SvcType_WeightMethod] DEFAULT (1) NOT NULL;
...
GO
PRINT N'Creating [core].[AddOnSvc#TotalUnfinished]...';
...
GO
PRINT N'Creating [tms].[WeightMethod#Type]...';


GO
--Smile
CREATE FUNCTION [tms].[WeightMethod#Type]()
RETURNS TABLE
WITH SCHEMABINDING, ENCRYPTION
AS RETURN
(
...
GO
PRINT N'Creating [hub].[Parcel$ForUSDEndOfDay]...';
...
BEGIN
	...
	BEGIN TRY
		...
		,		@postCourier E8=(select PostCourier from core.RefNbr#Type());
		...
		select	    x.ID, r.SvcClass, t.TrackingNbr, iif(r.SvcClass=@svcClassLW, k.PostCourierPrevious, k.PostCourierOriginal)
		from	@labelSpecs x join @picResult r on r.SeqNbr=x.Seq
		cross	apply core.RefNbr#Type()  k
		cross	apply tms.TrackingNbr#Make(x.SvcCode, x.MailerID, x.MailerSeq, x.ID) t
		;
		...
		from	core._RefNbr o
		join	@postCourierSpec r on r.MatterID=o.MatterID
		where	o.Type=@postCourier
		;
...
END
GO
PRINT N'Altering [api].[PostCourierApi$Dequeue]...';


GO
-- Aimee, Smile
ALTER PROCEDURE [api].[PostCourierApi$Dequeue](@result tvp out)
--WITH ENCRYPTION
AS
BEGIN
	SET NOCOUNT    ON;
	...
	BEGIN TRY
		...
		,		@qresult tvp;
		execute	core.OutboundQ#Dequeue @source=@source, @qtype=@qtype, @result=@qresult out;

		with cte(text) as
		(
			select	[text()]=concat(k.Many, x.ID, k.Duad, x.LastMilerID) 
			from	tvp.Spr#Const() k, tvp.Quad#Slice(@qresult, default, default) q
...
BEGIN
	SET NOCOUNT    ON;
		...
		--cross	apply tms.SvcFacility#For(t.Source, p.SvcClass, left(t.ZoneCode, 3)) f
		--outer	apply tms.SvcZone#For(t.Source, p.SvcClass, f.ImportZip3, left(t.ZoneCode, 3)) z
		--;
END
GO
...
BEGIN
	...
	BEGIN TRY
		...
		begin
			...
			declare	@newPicParcelIDSeqs I64Seqs;
			insert	@newPicParcelIDSeqs(ID, Seq) 
			select	MatterID, next value for core.MatterSeq
			...
			declare	@picSpec tms.PicSpec; insert @picSpec
			(		  MeasuredWt,  SvcType, Zip3,                Plus2)
			select	t.MeasureWt, p.SvcType, left(p.ZoneCode, 3), substring(p.ZoneCode, 4, 2)
			...
			from	@postCourierSpec x join @parcelSpec t on t.MatterID=x.ID
