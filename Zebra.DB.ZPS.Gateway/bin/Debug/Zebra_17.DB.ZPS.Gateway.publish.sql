...
BEGIN
	...
	BEGIN TRY
		...
		declare	@picSpec tms.PicSpec; insert @picSpec
		(		 MeasuredWt,   SvcType, Zip3,                Plus2)
		select	x.MeasureWt, p.SvcType, left(p.ZoneCode, 3), substring(p.ZoneCode, 4, 2)
		...
		insert	core._RefNbr
		(		MatterID, Type,          Number)
		select	x.ID,     r.PostCourier, x.PostCourier
		from	@postCourierSpec x cross apply core.RefNbr#Type() r;
		
		-- 5	Upd Parcel Weight
		update	o set o.Weight=n.MeasureWt, SvcClass=n.SvcClass
		from	shpt._Parcel o join @parcelSpec n on o.ID=n.ID;

		-- 6	Transit
		declare	@actionID    I32=(select HubMeasure from core.Action#ID())
		,		@idsInCsv    tvp
		,		@idArray     I64Array;
		insert	@idArray(ID) select ID from @parcelSpec;
