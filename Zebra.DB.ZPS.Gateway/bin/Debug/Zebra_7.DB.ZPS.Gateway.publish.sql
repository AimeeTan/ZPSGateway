...
(@contractID INT, @onZoneCode3 CHAR (3), @toZoneCode3 CHAR (3), @toPlus2 CHAR (2), @svcClass TINYINT, @measuredWt REAL)
RETURNS TABLE 
WITH ENCRYPTION
AS
RETURN 
...
GO
PRINT N'Altering [whse].[Ramp#Routing]...';


GO
--AaronLiu
ALTER FUNCTION [whse].[Ramp#Routing] ()
RETURNS TABLE
WITH SCHEMABINDING
AS RETURN
(
...
GO
PRINT N'Creating [lc].[@--------------------#lc]...';


GO
--AaronLiu
CREATE FUNCTION [lc].[@--------------------#lc]()
RETURNS TABLE
AS RETURN (select null as [Nil])
/*
	Document comes here.
...
GO
PRINT N'Creating [xpd].[Sack$Verify]...';
...
GO
--Daxia
ALTER FUNCTION [shpt].[Parcel#LedgerByFactor](@parcelIDs dbo.I64Array readonly)
RETURNS TABLE
WITH SCHEMABINDING, ENCRYPTION
AS RETURN
(
...
GO
PRINT N'Creating [lc].[Parcel$Sorting]...';
...
END
GO
PRINT N'Creating [hub].[Parcel$UpdAddOnSvc]...';
...
*/
-- Aimee
CREATE PROCEDURE [hub].[Parcel$UpdAddOnSvc](@slip tvp, @tenancy tvp)
WITH ENCRYPTION
AS
BEGIN
	SET NOCOUNT    ON;
...
GO
--Daxia
ALTER PROCEDURE [shpt].[Parcel#BillForFactor](@parcelIDs dbo.I64Array readonly)
WITH ENCRYPTION--
AS
BEGIN
	SET	NOCOUNT    ON;
...
END
GO
PRINT N'Creating [lc].[Parcel$CheckIn]...';
...
END
GO
PRINT N'Creating [lc].[Parcel$Measure]...';
...
BEGIN
	...
	BEGIN TRY
		...
		begin
			...
			where	x.MatterID=@matterID and x.Type in(t.PostCourierPrevious, t.PostCourierPrevious)
			;
			...
			select	@matterID,     0,   /*k.Overlabel*/106, getutcdate(), N''
