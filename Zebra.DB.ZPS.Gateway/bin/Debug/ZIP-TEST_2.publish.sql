...
GO
PRINT N'Dropping [hub].[Parcel$UpdAddOnSvc]...';


GO
DROP PROCEDURE [hub].[Parcel$UpdAddOnSvc];


GO
PRINT N'Dropping [hub].[Parcel$UpdAddOnSvcForUSD]...';


GO
DROP PROCEDURE [hub].[Parcel$UpdAddOnSvcForUSD];
...
GO
PRINT N'Creating [svc].[Parcel$ForFedEx]...';
...
(
	...
	(
		...
		),	p.POA, p.Source, p.SvcClass, p.ZoneCode
		from	tvp.I64#Slice(@parcelIdInCsv) x
		cross	apply tvp.Spr#Const()         k
		join	shpt.Parcel#Base()            p on p.ID=x.ID
		join	tms.SvcClass#Raw()            s on s.ID = p.SvcClass 
		cross	apply core.RefNbr#Type()                t
		cross	apply core.RefNbr#Of(p.ID,  t.MIT)      r
		cross	apply core.RefInfo#Type()               i
		cross	apply core.RefInfo#Of(p.ID, i.CneeInfo) c
		cross	apply tms.BarcodeNbr#Make(p.ZoneCode, r.Number) b
	)
	select	f.POA, f.FacilityInfo, f.ShprInfo, ParcelInfo=stuff(c.ParcelInfo, 1, 3, N'')
	from	cte c
	cross	apply tms.SvcFacility#For(c.Source, c.SvcClass, left(c.ZoneCode, 3)) f
	where	c.POA=f.POA
)
GO
...
AS RETURN
(
	select	x.RcvHubID, RcvHubAlias=t.Alias
	,		TotalParcelCnt  =isnull(count(*), 0)
	,		TotalAddOnSvcCnt=isnull(sum(iif(a.OperatorID=0, 0, 1)), 0)
	...
	where	x.Source=s.USD and x.StateID=38661/*e.CfmOutGated*/
...
END
GO
PRINT N'Creating [api].[PostCourierApi$Dequeue]...';
...
END
GO
PRINT N'Altering [bo].[Account$DepositFor]...';
...
*/
--Smile
ALTER PROCEDURE [bo].[Account$DepositFor](@slip tvp, @context tvp, @tenancy tvp)
--WITH ENCRYPTION
AS
BEGIN
	SET	NOCOUNT    ON;
