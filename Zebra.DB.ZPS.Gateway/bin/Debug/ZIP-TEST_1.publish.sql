...
GO
PRINT N'Dropping [shpt].[Parcel#LedgerByFactor]...';


GO
DROP FUNCTION [shpt].[Parcel#LedgerByFactor];
...
GO
PRINT N'Dropping [svc].[Parcel$FedExOutboundDequeue]...';


GO
DROP FUNCTION [svc].[Parcel$FedExOutboundDequeue];
...
GO
PRINT N'Dropping [svc].[Parcel$PushFedExOutboundDequeue]...';


GO
DROP FUNCTION [svc].[Parcel$PushFedExOutboundDequeue];
...
GO
PRINT N'Removing schema binding from [shpt].[SackMft#Raw]...';
...
GO
PRINT N'Dropping [api].[OutboundQ$DequeueMftPostCourierAPI]...';


GO
DROP PROCEDURE [api].[OutboundQ$DequeueMftPostCourierAPI];
...
GO
PRINT N'Altering [shpt].[_SackMft]...';


GO
ALTER TABLE [shpt].[_SackMft]
    ADD [MawbWt] FLOAT (53) DEFAULT (0) NOT NULL;
...
END
GO
PRINT N'Adding schema binding to [shpt].[SackMft#Raw]...';
...
GO
PRINT N'Creating [core].[Hub#ByPOA]...';
...
(
	...
	,		OperationDate   =isnull(convert(varchar(10), dateadd(hour, t.UtcOffset, x.StatedOn),120), N'0001-01-01')
	from	shpt.Parcel#Base()         x
	join	core.Tenant#Raw()          t on t.ID=x.RcvHubID
	...
	,		convert(varchar(10), dateadd(hour, t.UtcOffset, x.StatedOn),120)
)
GO
...
GO
--Daxia
CREATE FUNCTION [shpt].[Parcel#LedgerByFactor](@parcelIDs dbo.I64Array readonly, @hubID int)
RETURNS TABLE
WITH SCHEMABINDING, ENCRYPTION
AS RETURN
(
...
BEGIN
	...
	BEGIN TRY
		...
		outer	apply core.AddOnSvc#Exists(x.ID) a
